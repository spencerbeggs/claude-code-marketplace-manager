...
export const parseInputs: Effect.Effect<ParsedInputs, InvalidInputError | Config.ConfigError> = Effect.gen(
	function* () {
		const name = yield* Cfg.string("name").pipe(Cfg.withDefault(""));
		const url = yield* Cfg.string("url").pipe(Cfg.withDefault(""));
		const path = yield* Cfg.string("path").pipe(Cfg.withDefault(""));
		const sha = yield* Cfg.string("sha").pipe(Cfg.withDefault(""));
		const json = yield* Cfg.string("json").pipe(Cfg.withDefault(""));
		...
		const baseBranch = emptyToNull(yield* Cfg.string("base-branch").pipe(Cfg.withDefault("")));
		const branch = yield* Cfg.string("branch").pipe(Cfg.withDefault("chore/repin-plugins"));
		const commitMessage = emptyToNull(yield* Cfg.string("commit-message").pipe(Cfg.withDefault("")));
		const prTitle = emptyToNull(yield* Cfg.string("pr-title").pipe(Cfg.withDefault("")));
		const prBody = emptyToNull(yield* Cfg.string("pr-body").pipe(Cfg.withDefault("")));

		const autoMergeRaw = yield* Cfg.string("auto-merge").pipe(Cfg.withDefault("rebase"));
